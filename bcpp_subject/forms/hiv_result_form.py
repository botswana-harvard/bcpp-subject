...
class HivResultForm (SubjectModelFormMixin):
    ...
    def clean(self):
        ...
        if (cleaned_data.get('hiv_result') == 'Declined'
                and not cleaned_data.get('why_not_tested')):
            raise forms.ValidationError(
                'If participant has declined testing, provide reason '
                'participant declined testing')

        # validating when testing not performed
        if (cleaned_data.get('hiv_result') == 'Not performed'
                and cleaned_data.get('why_not_tested')):
            raise forms.ValidationError(
        ...
        result_not_performed = (
            cleaned_data.get('hiv_result') == 'Not performed')
        if ((result_declined or result_not_performed)
                and cleaned_data.get('hiv_result_datetime')):
            raise forms.ValidationError(
                {'hiv_result_datetime': 'This field is not required.'})

        # testing done but giving reason why not done
        if (cleaned_data.get('hiv_result') in [POS, NEG, IND]
                and cleaned_data.get('why_not_tested')):
            raise forms.ValidationError(
                {'why_not_tested': 'This field is not required.'})
        ...
        if (cleaned_data.get('hiv_result') not in [POS, NEG, IND]
                and cleaned_data.get('insufficient_vol') in [YES, NO]):
            raise forms.ValidationError(
                'No blood drawn.  You do not need to indicate if volume '
                'is sufficient. Got {0}'.format(
                    cleaned_data.get('insufficient_vol')))
        if(
           cleaned_data.get('hiv_result') in [POS, NEG, IND] and
           cleaned_data.get('blood_draw_type') not in ['capillary', 'venous']):
            raise forms.ValidationError(
                'Blood was drawn. Please indicate the type.')
        return cleaned_data
    ...
    def validate_hiv_status_nd_blood_draw_type(self):
        ...
        if(
           ...
           cleaned_data.get('insufficient_vol') == NOT_APPLICABLE):
            raise forms.ValidationError(
                'Please indicate if the capillary tube has sufficient volume.')
        if (cleaned_data.get('blood_draw_type') == 'venous'
                and cleaned_data.get('insufficient_vol') in [YES, NO]):
            raise forms.ValidationError(
                'Venous blood drawn.  You do not need to indicate if volume is '
                'sufficient. Got {0}'.format(cleaned_data.get(
                    'insufficient_vol')))
