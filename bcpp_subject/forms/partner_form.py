...
class BasePartnerForm (SubjectModelFormMixin):
    ...
    def clean(self):
        ...
        if (cleaned_data.get('first_partner_hiv') == 'negative' and
                cleaned_data.get('first_haart') in self.yes_no_unsure_options):
            raise forms.ValidationError(
                'Do not answer this question if partners HIV status is '
                'known to be negative')
        if (cleaned_data.get('first_partner_hiv') == 'I am not sure' and
                cleaned_data.get('first_haart') in self.yes_no_unsure_options):
            raise forms.ValidationError(
        ...
        if cleaned_data.get('third_last_sex_calc'):
            ...
            if (cleaned_data.get('third_last_sex', None) == 'Months' and
                ...
                if (cleaned_data.get('third_last_sex_calc') > 12):
                    raise forms.ValidationError(
                        'if last time of sex is in months, then months in a '
                        'year cannot exceed 12')
        # validating number of months and days
        if cleaned_data.get('first_first_sex_calc'):
            if (cleaned_data.get('first_first_sex', None) == 'Days'):
                if cleaned_data.get('first_first_sex_calc') > 31:
                    raise forms.ValidationError(
                        'if first time of sex is in days, then days '
                        'cannot exceed 31')
            if (cleaned_data.get('first_first_sex', None) == 'Months'):
                if (cleaned_data.get('first_first_sex_calc') > 12):
                    raise forms.ValidationError(
                        'if first time of sex is in months, '
                        'then months in a year cannot exceed 12')
