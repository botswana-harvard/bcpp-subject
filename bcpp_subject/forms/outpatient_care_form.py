...
class OutpatientCareForm (SubjectModelFormMixin):
    ...
    def clean(self):
        ...
        if cleaned_data.get('govt_health_care'):
            ...
            if cleaned_data.get('care_reason') == 'Pregnancy' and cleaned_data.get('subject_visit').household_member.gender == MALE:
                raise forms.ValidationError('If participant is male, pregnancy cannot be choosen')
            return cleaned_data
