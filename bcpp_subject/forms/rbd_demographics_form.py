...
class RbdDemographicsForm (SubjectModelFormMixin):
    ...
    def clean(self):
        ...
        if cleaned_data.get('marital_status') != 'Married' and cleaned_data.get('num_wives'):
            raise forms.ValidationError(
                'If participant is not married, do not give number of wives')
        if cleaned_data.get('marital_status') != 'Married' and cleaned_data.get('husband_wives'):
            raise forms.ValidationError(
                'If participant is not married, the number of wives is not required')
        ...
        if cleaned_data.get('marital_status') == 'Married':
            ...
            if husband_wives > 0 and num_wives > 0:
                raise forms.ValidationError(
                    'You CANNOT fill in both for WOMEN & MEN. Choose one')
            if not (husband_wives > 0 or num_wives > 0):
                raise forms.ValidationError(
                    'If participant is married, write the number of wives '
                    'for the husband [WOMEN:] OR the number of '
