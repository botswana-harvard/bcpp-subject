...
class TestEssSurveyRuleGroups(SubjectMixin, RuleGroupMixin, TestCase):
    ...
    def test_hiv_car_adherence_and_pima1(self):
        ...
        self.assertEqual(self.crf_metadata_obj(
            'bcpp_subject.pima', NOT_REQUIRED, E0,
    ...
    def test_newly_pos_and_not_art_bhs(self):
        ...
        self.assertEqual(self.crf_metadata_obj(
            'bcpp_subject.pima', REQUIRED, E0, self.subject_identifier).count(), 1)

    def test_not_known_pos_runs_hiv_and_cd4(self):
        """If not a known POS, requires HIV and CD4 (until today's
        result is known).
        """
        self.subject_identifier = self.subject_visit_male.subject_identifier
        # add hivtestreview,
        ...
        def assert_crfs():
            self.assertEqual(
                self.crf_metadata_obj(
                    ...
                    KEYED, E0, self.subject_identifier).count(), 1)
            self.assertEqual(
                self.crf_metadata_obj(
    ...
    def test_known_pos_completes_hiv_care_adherence(self):
        """If known POS (not including today's test), requires
        hiv_care_adherence.
        """
        self.subject_identifier = self.subject_visit_male.subject_identifier
    ...
    def test_known_neg_requires_hiv_test_today(self):
        ...
        self.assertEqual(
            self.crf_metadata_obj(
                'bcpp_subject.hivtestreview', KEYED, E0,
                self.subject_identifier).count(), 1)
        self.assertEqual(
            self.crf_metadata_obj(
                'bcpp_subject.hivresult', REQUIRED, E0,
                self.subject_identifier).count(), 1)

    def test_known_pos_does_not_require_hiv_test_today(self):
        """If previous result is POS, do not need to test
        today (HivResult).
    ...
    def test_hiv_tested_forms(self):
        ...
        self.assertEqual(
            self.crf_metadata_obj(
                'bcpp_subject.hivtested', REQUIRED,
                E0, self.subject_identifier).count(), 1)
        self.assertEqual(
            self.crf_metadata_obj(
...
#         hiv_testing_history.save()
    ...
    def test_hiv_care_adherance_for_verbal_posetive_only(self):
        """HivCareAdharance form should be made available
        any verbal positive, not considering availability or
        lack thereof documentation.
        """
        self.subject_identifier = self.subject_visit_male.subject_identifier
    ...
    def test_known_pos_on_art_with_doc_requires_cd4_only(self):
        """If previous result is POS on art with doc evidence,
        do not run HIV or CD4.
    ...
    def test_known_pos_no_art_but_has_doc_requires_cd4_only(self):
        ...
        self.assertEqual(
            self.crf_metadata_obj(
                'bcpp_subject.hivcareadherence', REQUIRED, E0,
                self.subject_identifier).count(), 1)
        ...
        self.assertEqual(
            self.crf_metadata_obj(
                'bcpp_subject.hivcareadherence', KEYED, E0,
                self.subject_identifier).count(), 1)
        self.assertEqual(
            self.crf_metadata_obj(
                'bcpp_subject.hivresult', NOT_REQUIRED, E0,
                self.subject_identifier).count(), 1)
        self.assertEqual(
            self.crf_metadata_obj(
                'bcpp_subject.pima', NOT_REQUIRED, E0,
    ...
    def test_elisaresult_behaves_like_todayhivresult(self):
        """When an elisa result is keyed in, a +ve result should
        result in RBD and VL being REQUIRED just like Today's
        HivResult.
        """
        self.subject_identifier = self.subject_visit_male.subject_identifier
        ...
        self.assertEqual(
            self.crf_metadata_obj(
                'bcpp_subject.elisahivresult', NOT_REQUIRED, E0,
                self.subject_identifier).count(), 1)
        ...
        self.assertEqual(
            self.crf_metadata_obj(
                'bcpp_subject.elisahivresult', REQUIRED, E0,
                self.subject_identifier).count(), 1)
        ...
        self.assertEqual(
            self.crf_metadata_obj(
                'bcpp_subject.elisahivresult', KEYED, E0,
                self.subject_identifier).count(), 1)
        self.assertEqual(self.requisition_metadata_obj(
            REQUIRED, E0, VIRAL_LOAD, self.subject_identifier).count(), 1)
        self.assertEqual(
            self.requisition_metadata_obj(
                REQUIRED, E0, RESEARCH_BLOOD_DRAW,
                self.subject_identifier).count(), 1)

    def test_normal_circumsition_in_y1(self):
        self.subject_identifier = self.subject_visit_male.subject_identifier

        def assert_circumcision(
                circumcision_entry_status,
                uncircumcised_entry_status,
                circumcised_entry_status):
            self.assertEqual(
                self.crf_metadata_obj(
                    'bcpp_subject.circumcision',
                    circumcision_entry_status, E0,
                    self.subject_identifier).count(), 1)
            self.assertEqual(
                self.crf_metadata_obj(
                    'bcpp_subject.uncircumcised',
                    uncircumcised_entry_status, E0,
                    self.subject_identifier).count(), 1)
            self.assertEqual(
                self.crf_metadata_obj(
                    'bcpp_subject.circumcised',
                    circumcised_entry_status, E0,
                    self.subject_identifier).count(), 1)
        assert_circumcision(REQUIRED, REQUIRED, REQUIRED)
        ...
        assert_circumcision(KEYED, NOT_REQUIRED, REQUIRED)
        ...
        assert_circumcision(KEYED, REQUIRED, NOT_REQUIRED)

        circumcition.circumcised = NOT_SURE
        circumcition.save()

        assert_circumcision(KEYED, NOT_REQUIRED, NOT_REQUIRED)
    ...
    def hiv_pos_nd_art_naive_at_bhs(self):
        ...
        Pima, RBD and VL required. Then Key RBD for later use
        in Annual survey.
        """
        self.subject_identifier = self.subject_visit_male.subject_identifier
        # Known POS in E0
        ...
        self.assertEqual(self.requisition_metadata_obj(
            'bcpp_subject.subjectrequisition', REQUIRED, E0,
        ...
        self.assertEqual(
            self.requisition_metadata_obj(
                KEYED, E0, RESEARCH_BLOOD_DRAW, self.subject_identifier).count(), 1)
    ...
    def test_hiv_pos_nd_not_on_art_at_bhs(self):
        ...
        self.assertEqual(
            self.requisition_metadata_obj(
                REQUIRED, E0, RESEARCH_BLOOD_DRAW, self.subject_identifier).count(), 1)
