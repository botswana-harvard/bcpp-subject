...
class TestHicEnrollmentForm(SubjectMixin, TestCase):
    ...
    def setUp(self):
        ...
        mommy.make_recipe(
            'bcpp_subject.subjectrequisition', subject_visit=self.bhs_subject_visit_female, report_datetime=get_utcnow(),
        ...
        mommy.make_recipe(
            'bcpp_subject.hivresult', subject_visit=self.bhs_subject_visit_female, report_datetime=get_utcnow(),
            hiv_result=NEG
        )
        mommy.make_recipe(
            'bcpp_subject.elisahivresult', subject_visit=self.bhs_subject_visit_female, report_datetime=get_utcnow(),
            hiv_result=NEG)
        mommy.make_recipe(
            'bcpp_subject.residencymobility', subject_visit=self.bhs_subject_visit_female, report_datetime=get_utcnow(),
    ...
    def test_is_permanent_resident(self):
        ...
        residencymobility = ResidencyMobility.objects.get(subject_visit=self.bhs_subject_visit_female)
    ...
    def test_is_citizen_or_spouse(self):
        enrollment_checklist = EnrollmentChecklist.objects.get(
            household_member__subject_identifier=self.bhs_subject_visit_female.subject_identifier)
        ...
        subject_consent = SubjectConsent.objects.get(subject_identifier=self.bhs_subject_visit_female.subject_identifier)
    ...
    def test_is_locator_information(self):
        ...
        subject_locator = SubjectLocator.objects.get(subject_identifier=self.bhs_subject_visit_female.subject_identifier)
        for field in fields:
    ...
    def test_get_hiv_status_today(self):
        hiv_result = HivResult.objects.get(subject_visit=self.bhs_subject_visit_female)
