...
from ..models.enrollment import (
    EnrollmentBhs, EnrollmentAhs, EnrollmentEss)
...
class TestSurveyAndConsent(SubjectMixin, TestCase):
    ...
    def setUp(self):
        super().setUp()
        for survey_schedule_object in site_surveys.get_survey_schedules(
            ...
            household_structure = self.make_household_ready_for_enumeration(
                ...
                make_hoh=False)
            self.assertEqual(
                household_structure.report_datetime, report_datetime)
        ...
        self.household_members = HouseholdMember.objects.all().order_by(
            'report_datetime')
        self.assertEqual(self.household_members.count(), 3)
        for index, survey_schedule_name in enumerate(
                self.survey_schedule_names):
            self.assertEqual(
                self.household_members[index].survey_schedule,
                survey_schedule_name)
    ...
    def test_subject_consent_attrs(self):
        ...
        for household_member in self.household_members:
            ...
            subject_consents.append(subject_consent)

        for index, subject_consent in enumerate(subject_consents):
            self.assertEqual(
                subject_consent.survey_schedule,
                self.survey_schedule_names[index])
