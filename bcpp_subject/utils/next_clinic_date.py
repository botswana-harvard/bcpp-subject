...
def next_clinic_date(community_clinic_days, base_datetime=None,
                     allow_same_day=None, subtract=None):
    ...
    if community_clinic_days:
        ...
        for DAY in community_clinic_days.days:
            if allow_same_day:
                clinic_dates.append(
                    base_datetime + relativedelta(weekday=DAY(+1)))
            elif base_datetime + relativedelta(weekday=DAY(+1)) != base_datetime:
                clinic_dates.append(
                    base_datetime + relativedelta(weekday=DAY(+1)))
        if not clinic_dates:
            clinic_dates.append(
                base_datetime + relativedelta(days=1, weekday=DAY(+1)))
        next_clinic_datetime = datetime(
            min(clinic_dates).year,
            min(clinic_dates).month,
            min(clinic_dates).day, 7, 30, 0)
